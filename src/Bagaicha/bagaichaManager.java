 ...
 * @author digitech
 */
public class bagaichaManager {
    connectorBagaicha mysql = new     connectorBagaicha();
    ...
    public void Product(ProductDetails user) {
        Connection conn = mysql.openConnection();
        
        String sql = "INSERT INTO Product (Product_id, Product_name, Quantity, Price) VALUES (?, ?, ?, ?)";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, user.getProduct_id());
            pstmt.setString(2, user.getProduct_name());
            pstmt.setInt(3, user.getQuantity());
            pstmt.setDouble(4, user.getPrice());
            pstmt.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(ProductDetails.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
